...
code_words = input('Введите строку: ')  # Введите строку: ssbbbsssbc

first_word = 0
second_word = 0

for symbol in code_words:
    if symbol == 's':  # считаем буквы S
        first_word += 1
    elif first_word > second_word:  # если первая группа символов больше второй,
        second_word = first_word  # то переприсвоить значения
        first_word = 0  # обнуляем первую группу
    else:  # когда буквы S заканчиваются
        first_word = 0  # обнуляем первую группу

print('Самая длинная последовательность:', second_word)
